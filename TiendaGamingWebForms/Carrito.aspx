...
<asp:Content ID="BodyContent" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <%-- Layout Principal (Tabla a la izq, Resumen a la der) --%>
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
                        ...
                        <asp:Repeater ID="rptCarrito" runat="server">
                            <ItemTemplate>
                                <tr class="flex flex-col md:table-row border-b md:border-t border-gray-200 dark:border-t-[#323267] p-4 md:p-0">
                                    ...
                                    <%-- Precio --%>
                                    <td class="h-auto md:h-[96px] px-0 md:px-4 py-2 text-gray-600 dark:text-[#9292c9] text-sm font-normal leading-normal">
                                    ...
                                    <td class="h-auto md:h-[96px] px-0 md:px-4 py-2 text-gray-600 dark:text-[#9292c9] text-sm font-normal leading-normal">
                                        <div class="flex items-center">
                                            <button type="button" class="size-8 rounded-l-md border border-gray-300 dark:border-[#323267]">-</button>
                                            <input class="w-12 h-8 text-center border-y border-gray-300 dark:border-[#323267] bg-transparent dark:text-white focus:outline-none focus:ring-0" type="text" value='<%# Eval("Cantidad") %>'/>
                                            <button type="button" class="size-8 rounded-r-md border border-gray-300 dark:border-[#323267]">+</button>
                                        </div>
                                    </td>
                                    ...
                                    <%-- Botón Eliminar --%>
                                    <td class="h-auto md:h-[96px] px-0 md:px-4 py-2 text-sm font-bold leading-normal tracking-[0.015em]">
                                        <button type="button" class="text-gray-500 dark:text-[#9292c9] hover:text-red-500">
        ...
        <%-- Columna Derecha: Resumen del Pedido (Sigue ESTÁTICO por ahora) --%>
                ...
                <div class="space-y-3 mb-6">
                    <div class="flex justify-between gap-x-6 py-2">
                        <p class="text-gray-500 dark:text-[#9292c9] text-sm font-normal leading-normal">Subtotal</p>
                            <asp:Literal ID="litSubtotal" runat="server" Text="$0.00" CssClass="text-sm font-medium leading-normal text-right dark:text-white" />
                    </div>
                    <%-- (Resto de los ítems estáticos: Envío, Impuestos, Total) --%>
                </div>
                <hr class="border-gray-200 dark:border-[#323267] my-4"/>
                <div class="flex justify-between gap-x-6 py-2 mb-6">
                    <p class="text-lg font-bold dark:text-white">Total</p>
                    <asp:Literal ID="litTotal" runat="server" Text="$0.00" CssClass="text-lg font-bold leading-normal text-right dark:text-white" />
                </div>
                <%-- Código de Descuento --%>
                <div class="flex flex-col gap-4 mb-6">
                    <p class="text-base font-medium leading-normal pb-2 dark:text-white">¿Tienes un código de descuento?</p>
                    <div class="flex items-stretch">
                        <asp:TextBox ID="txtCodigoDescuento" runat="server" CssClass="... (clases) ..." placeholder="Ingresa tu código"></asp:TextBox>
                        <asp:Button ID="btnAplicarDescuento" runat="server" Text="Aplicar" CssClass="... (clases) ..." />
                    </div>
                </div>
                <asp:Button ID="btnProcederPago" runat="server" Text="Proceder al Pago" CssClass="... (clases) ..." />
            </div>
        </div>
    </div> 
</asp:Content>
